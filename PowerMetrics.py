...
def collect_devices(n):
    ...
    for i in range(1, n + 1):
        ...
        devices.append({"name": name, "watts": watts, "hours": hours, "qty": qty, "kwh_day": kwh_day})
    return devices
...
def summary_and_report(devices, days, tariff):
    ...
    print("-" * total_width)
    print(pad(h1, w1), "|", pad(h2, w2), "|", pad(h3, w3), "|", pad(h4, w4), "|", pad(h5, w5))
    print("-" * total_width)
    for d in devices:
        print(pad(d["name"], w1), "|", pad(str(int(d["watts"])), w2), "|", pad(f'{d["hours"]:.1f}', w3), "|", pad(str(d["qty"]), w4), "|", pad(f'{d["kwh_day"]:.3f}', w5))
    print("-" * total_width)
    # Summary (minimal color)
    print()
    print(f"{GRN}{BOLD}Summary{RST}")
    print(f"  Total daily consumption      : {total_kwh_day:.3f} kWh")
    print(f"  Billing cycle consumption    : {total_kwh_month:.3f} kWh (days={days})")
    print(f"  Estimated bill @ ₹{tariff:.2f}/kWh : {YEL}₹{est_bill:,.2f}{RST}")
    print(f"  Highest daily consumer       : {top['name']} ({top['kwh_day']:.3f} kWh/day)")
    print()
    ...
    if top["hours"] > 8 and top["watts"] > 200:
        print(f"  • Try reducing {top['name']} hours or enable energy-saver settings.")
    if tariff > 9:
    ...
    for d in devices:
        lines.append(f"{d['name']:20} {int(d['watts']):7d} {d['hours']:7.1f} {d['qty']:5d} {d['kwh_day']:10.3f}")
    lines += [
        "",
